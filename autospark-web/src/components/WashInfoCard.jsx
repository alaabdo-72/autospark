...
export default function WashInfoCard({ statusLabel }) {
  const { washName, exteriorMinutes, interiorMinutes } = useServiceConfig()
  ...
  return (
    <div className="px-5 mt-5">
      ...
      <div className="rounded-2xl border border-brand-500 bg-brand-50 px-4 py-3">
        <p className="text-sm font-semibold text-slate-800">{washName}</p>
        <div className="flex justify-between text-xs text-slate-500 mt-2">
          <span>Exterior wash by machine</span>
          <span>{exteriorMinutes} min</span>
        </div>
        <div className="flex justify-between text-xs text-slate-500 mt-1">
          <span>Vacuuming & Interior Detailing</span>
          <span>{interiorMinutes} min</span>
        </div>
